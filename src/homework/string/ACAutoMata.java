...
public class ACAutoMata {
        ...
        for(int i = 0; i < n; i++) {
            ...
            while ( tmp != root) {
                if (tmp.isEndingChar == true) {
                    //System.out.println("Start from " + (i - p.length + 1)+" "+p.length);
                    int start = i - p.length + 1;
                    matchedList.add(text.substring(start,start+p.length));
                    p = root;
                    break;
                }
                tmp = tmp.fail;
    ...
    public static List<String> match(String text, String[] patterns) {
